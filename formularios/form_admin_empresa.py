...
class AdminFormularioEmpresa(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Búsqueda y Modificación de Empresa")


        # Crear los campos de entrada para el nombre de usuario y los resultados
        self.txt_nombre_busqueda = QLineEdit()
        self.txt_nombre_busqueda.setPlaceholderText("Nombre de Empresa")
        ...
        self.btn_eliminar = QPushButton("Borrar Empresa")
    ...
    def buscar_en_base_de_datos(self, nombre):
        ...
        try:
            ...
            consulta = "SELECT * FROM Empresa WHERE nempresa = ?"
            parametros = [nombre]
            ...
            CustomMessageBox.critical(self, "Error", f"Error al buscar empresas en la base de datos: {error}")
            return []
    ...
    def mostrar_resultados(self, usuarios_encontrados):
        ...
        if not usuarios_encontrados:
            mensaje = QLabel("No se encontraron empresas.")
        ...
        for campo_edit, valor in zip(self.campos_edit, usuario[1:]):
    ...
    def guardar_cambios(self):
        ...
        try:
            ...
            consulta = "UPDATE Empresa SET nempresa=?, direccion=?, telefono=?, correo=?, logo=?, descripcion=?, tipo=?, id_usuario=? WHERE nempresa = ?"
    ...
    def eliminar_usuario(self):
        ...
        if respuesta == CustomMessageBox.Yes:
            ...
            try:
                ...
                consulta = "DELETE FROM Empresa WHERE nempresa = ?"
                ...
                # Mostrar un mensaje de éxito
                CustomMessageBox.information(self, "Éxito", f"La empresa '{nombre_usuario}' ha sido eliminado correctamente.")
...
if __name__ == "__main__":
    ...
    ventana = AdminFormularioEmpresa()
    ventana.show()
    sys.exit(app.exec_())
